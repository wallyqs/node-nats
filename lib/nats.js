    ...
    while (!client.closed && client.inbound && client.inbound.length > 0) {
        switch (client.pstate) {
            ...
            case AWAITING_CONTROL:
                ...
                var pos = client.inbound.indexOf(CR_LF);
                if (pos == -1) {
                    // No full control line yet so skipping
                    return;
                }
                var buf = client.inbound.toString('binary', 0, pos+CR_LF_LEN);

                if ((m = MSG.exec(buf)) !== null) {
                    client.payload = {
                        subj: Buffer.from(m[1], 'binary').toString(client.encoding),
                        sid: parseInt(m[2], 10),
                        reply: m[4],
